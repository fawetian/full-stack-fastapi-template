...
- 📫 基于电子邮件的密码恢复。
- 📬 [Mailcatcher](https://mailcatcher.me) 用于本地开发期间的电子邮件测试。
...
如果你想要一个私有仓库，GitHub 不允许你简单地 fork 它，因为它不允许更改 fork 的可见性。
...
- 将新的 origin 设置为你新仓库的地址，从 GitHub 界面复制，例如：
...
你可以（并且应该）从 secrets 中将这些作为环境变量传递。
...
或者更好的是，如果你有 [`pipx`](https://pipx.pypa.io/)，你可以通过以下方式运行它：
...
**注意** `--trust` 选项是必需的，以便能够执行更新你的 `.env` 文件的[创建后脚本](https://github.com/fastapi/full-stack-fastapi-template/blob/master/.copier/update_dotenv.py)。

### 输入变量

Copier 会询问你一些数据，你可能希望在生成项目之前准备好。

但别担心，你可以稍后在 `.env` 文件中更新任何内容。
...
- `emails_from_email`:（默认：`"info@example.com"`）用于发送电子邮件的电子邮件帐户，你可以稍后在 .env 中设置它。
